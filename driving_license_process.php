...
<div id="middle">
	...
	<div id="content_left">
		...
		{
			$rec=mysql_query("select * from dr_no");
			$row=mysql_fetch_array($rec);
			$r=$row['dr_no'];
		}
		else
		{
			$rec=mysql_query("select * from driving_license order by dr_no desc");
			$row=mysql_fetch_array($rec);
			$r=$row['dr_no']+1;
		}
		
		mysql_query("insert into driving_license (category, cand_name, sex, dob, fname, mname, nationality, marital_code, religion, community, minority_status, qualcode, add1, add2, po, district, state, pincode, area_code, phone, mobile, email, confirmemail, date, dr_no) values ('".$_REQUEST['category']."', '".$_REQUEST['cand_name']."', '".$_REQUEST['sex']."', '".$_REQUEST['dob']."', '".$_REQUEST['fname']."', '".$_REQUEST['mname']."', '".$_REQUEST['nationality']."', '".$_REQUEST['marital_code']."', '".$_REQUEST['religion']."', '".$_REQUEST['community']."', '".$_REQUEST['minority_status']."', '".$_REQUEST['qualcode']."', '".$_REQUEST['add1']."', '".$_REQUEST['add2']."', '".$_REQUEST['po']."', '".$_REQUEST['district']."', '".$_REQUEST['state']."', '".$_REQUEST['pincode']."', '".$_REQUEST['area_code']."', '".$_REQUEST['phone']."',  '".$_REQUEST['mobile']."', '".$_REQUEST['email']."', '".$_REQUEST['confirmemail']."', '$date' , '$r')");
		
		
		mysql_query("insert into dl_status (category, cand_name, dob, email, date, dr_no) values ('".$_REQUEST['category']."', '".$_REQUEST['cand_name']."', '".$_REQUEST['dob']."', '".$_REQUEST['email']."', '$date' , '$r')");
		echo '<input type="hidden" value="'.$r.'" name="dr_no">';

		?>
		<form action="driving_license_process1.php" method="post" enctype="multipart/form-data" name="upload_conent_form">
		<table align="center" cellspacing="6" width="75%" border="0">
		  <tbody>
		  <tr><td>
 		<?php
		echo $r.'<input type="hidden" name="dr_no" value="'.$r.'"></td></tr>';
		?>
		</td></tr>
		  <tr>
