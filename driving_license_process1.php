...
<div id="header_wrapper">
	<div id="header">
		...
		<div id="header_right">
        	<div id="social_box">
                <ul>
					<li><div class="login"><a href="admin_login.php"><img src="images/login.png" height="32" width="81"></a></div> </li>
                </ul>
...
<div id="middle">
	<div id="middle_left">
		<div class="boxedB">
			...
			<ul  >
				<li><a target="_blank" href="#">LLR Practices and Mock Test</a></li>
				<li><a target="_blank" href="driving_license.html"> Online Application</a></li>
	...
	<div id="content_left">
    	...
    	<h3>Please keep the hard copy of form and Note down registration number : <?php echo $_REQUEST['dr_no']; ?> </h3>
		<div class="Hline">&nbsp;</div>
		<div class="cleaner_h10"></div>
		<?php
		//for photo upload
		if((($_FILES["photo"]["type"] == "image/gif") ||($_FILES["photo"]["type"] == "image/jpeg") || ($_FILES["photo"]["type"] == "image/pjpeg") || ($_FILES["photo"]["type"] == "image/png")) && ($_FILES["photo"]["size"]>0))
		{
			...
			else
			{
				...
				{
					move_uploaded_file($_FILES["photo"]["tmp_name"],"photo/" . $_REQUEST['dr_no'].".jpg");
				}
			}
		}
		else
		...
		if((($_FILES["sign"]["type"] == "image/gif") ||($_FILES["sign"]["type"] == "image/jpeg") || ($_FILES["sign"]["type"] == "image/pjpeg") || ($_FILES["sign"]["type"] == "image/png")) && ($_FILES["sign"]["size"]>0))
		{
			...
			{
				if(file_exists("sign/" . $_FILES["sign"]["name"]))
				{
					echo $_FILES["sign"]["name"] . "Already Exists...!!";
				}
				else
				{
					move_uploaded_file($_FILES["sign"]["tmp_name"],"sign/" . $_REQUEST['dr_no'].".jpg");
				}
			}
		}
		else
		...
		//for identity proof upload
		if((($_FILES["proof1"]["type"] == "image/gif") ||($_FILES["proof1"]["type"] == "image/jpeg") || ($_FILES["proof1"]["type"] == "image/pjpeg") || ($_FILES["proof1"]["type"] == "image/png")) && ($_FILES["proof1"]["size"]>0))
		{
			if($_FILES["proof1"]["error"] > 0)
			{
				echo "Return Code : " . $_FILES["proof1"]["error"] . "<br/>";
			}
			else
			{
				if(file_exists("proof1/" . $_FILES["proof1"]["name"]))
				{
					echo $_FILES["proof1"]["name"] . "Already Exists...!!";
				}
				else
				{
					move_uploaded_file($_FILES["proof1"]["tmp_name"],"proof1/" . $_REQUEST['dr_no'].".jpg");
				}
			}
		}
		else
		...
		if((($_FILES["proof2"]["type"] == "image/gif") ||($_FILES["proof2"]["type"] == "image/jpeg") || ($_FILES["proof2"]["type"] == "image/pjpeg") || ($_FILES["proof2"]["type"] == "image/png")) && ($_FILES["proof2"]["size"]>0))
		{
			if($_FILES["proof2"]["error"] > 0)
			{
				echo "Return Code : " . $_FILES["proof2"]["error"] . "<br/>";
			}
			else
			{
				if(file_exists("proof2/" . $_FILES["proof2"]["name"]))
				{
					echo $_FILES["proof2"]["name"] . "Already Exists...!!";
				}
				else
				{
					move_uploaded_file($_FILES["proof2"]["tmp_name"],"proof2/" . $_REQUEST['dr_no'].".jpg");
				}
			}
		}
		else
		...
		mysql_select_db("rto");
		$rec=mysql_query("select * from driving_license where dr_no=".$_REQUEST['dr_no']);
		...
		echo  "	<tr><td>Registration No.</td><td>".$row['dr_no']."</td></tr>
				...
				<tr><td>Father Name</td><td>".$row['fname']."</td></tr>
				<tr><td>Mother Name</td><td>".$row['mname']."</td></tr>
				...
				<tr><td>Photo</td><td><img src='photo/".$row['dr_no'].".jpg' height=200 width=200></td></tr>
				<tr><td>Signature</td><td><img src='sign/".$row['dr_no'].".jpg' height=200 width=200></td></tr>
