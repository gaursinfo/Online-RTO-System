...
<div id="middle">
	...
	<div id="content_left">
		<div class="col_w460">
    	<h3>Your National Permit Application is Submitted Succesfully...!!</h3>
		...
		{
			$rec=mysql_query("select * from permit_no");
			$row=mysql_fetch_array($rec);
			$r=$row['permit_no'];
		}
		else
		{
			$rec=mysql_query("select * from permit order by permit_no desc");
			$row=mysql_fetch_array($rec);
			$r=$row['permit_no']+1;
		}
		mysql_query("insert into permit (category, cand_name, add1, add2, po, district, state, nationality, pincode, area_code, phone, mobile, doj, dor, members, source, destination, type, date, permit_no) values ('".$_REQUEST['category']."','".$_REQUEST['cand_name']."','".$_REQUEST['add1']."', '".$_REQUEST['add2']."', '".$_REQUEST['po']."', '".$_REQUEST['district']."', '".$_REQUEST['state']."', '".$_REQUEST['nationality']."', '".$_REQUEST['pincode']."', '".$_REQUEST['area_code']."', '".$_REQUEST['phone']."',  '".$_REQUEST['mobile']."', '".$_REQUEST['doj']."', '".$_REQUEST['dor']."', '".$_REQUEST['members']."', '".$_REQUEST['source']."','".$_REQUEST['destination']."','".$_REQUEST['type']."', '$date', '$r')");
		
		mysql_query("insert into permit_status (category, cand_name, members, source, destination, type, date, permit_no) values ('".$_REQUEST['category']."', '".$_REQUEST['cand_name']."', '".$_REQUEST['members']."', '".$_REQUEST['source']."', '".$_REQUEST['destination']."', '".$_REQUEST['type']."', '$date' , '$r')");
		
    	echo "<h3>Please keep the hard copy of form and Note down registration number : ".$r."</h3>";
		$rec=mysql_query("select * from permit where permit_no=".$r);
		echo "<center>";
		...
		echo  "	<tr><td>Registration No.</td><td>".$row['permit_no']."</td></tr>
				...
				<tr><td>Date of Journey</td><td>".$row['doj']."</td></tr>
				<tr><td>Date of Return</td><td>".$row['dor']."</td></tr>
